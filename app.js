...
app.get("/agenda/", async (request, response) => {
  ...
  const formatDate = format(parseInt(date), "yyyy-MM-dd");
...
app.post("/todos/", async (request, response) => {
  ...
  const formatDate = format(parseInt(dueDate), "yyyy-MM-dd");
  ...
  } else if (!verificationDate(formatDate)) {
    response.status(400);
...
app.put("/todos/:todoId/", async (request, response) => {
  ...
  switch (true) {
    ...
    case requestBody.dueDate !== undefined:
      if (!verificationDate(requestBody.dueDate)) {
        response.status(400);
        response.send("Invalid Todo DueDate");
      } else {
